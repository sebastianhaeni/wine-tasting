...
class ConfigTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'WineTasting.Model.Map.ConfigTableMap';
     ...
     */
    const OM_CLASS = '\\WineTasting\\Model\\Config';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'WineTasting.Model.Config';
    ...
    /**
     * the column name for the idConfig field
     */
    const COL_IDCONFIG = 'config.idConfig';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('IdConfig', 'Name', 'Value', ),
        self::TYPE_CAMELNAME     => array('idConfig', 'name', 'value', ),
        self::TYPE_COLNAME       => array(ConfigTableMap::COL_IDCONFIG, ConfigTableMap::COL_NAME, ConfigTableMap::COL_VALUE, ),
        self::TYPE_FIELDNAME     => array('idConfig', 'name', 'value', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('IdConfig' => 0, 'Name' => 1, 'Value' => 2, ),
        self::TYPE_CAMELNAME     => array('idConfig' => 0, 'name' => 1, 'value' => 2, ),
        self::TYPE_COLNAME       => array(ConfigTableMap::COL_IDCONFIG => 0, ConfigTableMap::COL_NAME => 1, ConfigTableMap::COL_VALUE => 2, ),
        self::TYPE_FIELDNAME     => array('idConfig' => 0, 'name' => 1, 'value' => 2, ),
        self::TYPE_NUM           => array(0, 1, 2, )
    );
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\WineTasting\\Model\\Config');
        $this->setPackage('WineTasting.Model');
        $this->setUseIdGenerator(true);
        // columns
        $this->addPrimaryKey('idConfig', 'IdConfig', 'INTEGER', true, null, null);
        $this->addColumn('name', 'Name', 'VARCHAR', true, 200, null);
        $this->addColumn('value', 'Value', 'VARCHAR', false, 200, null);
    } // initialize()
    ...
    public static function getPrimaryKeyHashFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        if ($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('IdConfig', TableMap::TYPE_PHPNAME, $indexType)] === null) {
            return null;
        }

        return (string) $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('IdConfig', TableMap::TYPE_PHPNAME, $indexType)];
    }
    ...
    public static function getPrimaryKeyFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        return (int) $row[
            $indexType == TableMap::TYPE_NUM
                ? 0 + $offset
                : self::translateFieldName('IdConfig', TableMap::TYPE_PHPNAME, $indexType)
        ];
    }
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \WineTasting\Model\Config) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(ConfigTableMap::DATABASE_NAME);
            $criteria->add(ConfigTableMap::COL_IDCONFIG, (array) $values, Criteria::IN);
        }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria->containsKey(ConfigTableMap::COL_IDCONFIG) && $criteria->keyContainsValue(ConfigTableMap::COL_IDCONFIG) ) {
            throw new PropelException('Cannot insert a value for auto-increment primary key ('.ConfigTableMap::COL_IDCONFIG.')');
        }
